...
func (r *QBReponseToken) SetObtainedAt(t time.Time) {
	r.ObtainedAt = t
}
func (r *QBReponseToken) SetExpiresAt(t time.Time) {
	r.ExpiresAt = t.Add(time.Duration(r.ExpiresInSec) * time.Second)
}
