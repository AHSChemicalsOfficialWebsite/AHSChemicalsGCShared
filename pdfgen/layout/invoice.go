...
func NewInvoice(order *models.Order, invoiceNumber string) *Invoice {
	invoice := &Invoice{
		...
		PaymentDue:  order.UpdatedAt.AddDate(0, 0, 15).Format("January 2, 2006"),
		LateFeeDate: order.UpdatedAt.AddDate(0, 0, 30).Format("January 2, 2006"),
	}
...
func (i *Invoice) setTableValues(items []*models.CartItem) {
	...
	for _, item := range items {
		tableValues = append(tableValues, []string{
			...
			item.GetFormattedItemTotalPrice(),
		})
	}
...
func (i *Invoice) RenderToPDF() ([]byte, error) {
	...
	c.DrawFooter(fmt.Sprintf("If you have any questions or concerns about this invoice please contact us at %s", company_details.Email))
