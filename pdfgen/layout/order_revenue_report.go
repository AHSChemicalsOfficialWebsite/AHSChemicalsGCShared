...
func (orr *OrderRevenueReport) CreateTableValues(order *models.Order) {
	...
	for _, item := range order.Items {
		tableValues = append(tableValues, []string{item.Product.SKU, item.Product.GetFormattedDescription(), item.GetFormattedQuantity(), item.GetFormattedPrice(), item.Product.GetFormattedPurchasePrice(), item.GetFormattedItemTotalPrice(), item.Product.GetFormattedTotalPurchasePrice(item.Quantity), item.Product.GetFormattedTotalRevenuePerProduct(item.Price, item.Quantity)})
	}
