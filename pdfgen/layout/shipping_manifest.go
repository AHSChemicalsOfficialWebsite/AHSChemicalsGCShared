...
func NewShippingManifest(delivery *models.Delivery) *ShippingManifest {
	...
	shippingManifest := &ShippingManifest{
		...
		TotalNonHazardWeight: "0.00 gal",
		TotalHazardousWeight: delivery.Order.GetFormattedNetWeight(),
...
func (p *ShippingManifest) getTableValues(items []*models.OrderItem) {
	...
	for _, item := range items {
		tableValues = append(tableValues, []string{
			...
			"Yes", //All products are hazardous
			typeContainer,
			item.Product.GetFormattedDescription(),
			class,
			item.Product.SKU,
			item.Product.GetFormattedWeight(item.Quantity),
			"N/A",
			item.Product.GetFormattedWeight(item.Quantity),
		})
	}
...
func (p *ShippingManifest) RenderToPDF() ([]byte, error) {
	...
	//Footer
	c.DrawFooter(fmt.Sprintf("If you have any questions or concerns about this shipping manifest please contact us at %s", company_details.Email))
